...
DSH Desktop 是一个独立、轻量的 macOS 与 Windows 外壳，让 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 可以像普通应用一样打开。macOS 使用沉浸式窗口，不保留单独的标题栏占位；外壳不修改、复制或 fork 来源仓库的代码，运行内容始终来自 npm 官方包 `@deepseek-ai/dsh`。
...
```sh
npm run release:publish -- --windows-installer "dist-windows/DSH Desktop_0.1.3_x64-setup.exe" "本次更新说明"
```
...
```sh
npm run release:publish -- --dry-run --windows-installer "dist-windows/DSH Desktop_0.1.3_x64-setup.exe"
```
